...
// Pin: es-tooling/module-replacements v3.0.0.
...
// Mirrors resolveDocUrl in module-replacements src/util.ts.
function resolveDocUrl(url?: KnownUrl): string | undefined {
...
function buildEntry(manifest: ManifestName, mapping: ModuleMapping, descriptors: Record<string, ReplacementDescriptor>): GeneratedEntry | undefined {
  ...
  let docUrl = resolveDocUrl(mapping.url);
  ...
  for (const replacementId of mapping.replacements || []) {
    ...
    if (!docUrl) docUrl = resolveDocUrl(descriptor.url);
  }
...
async function main(): Promise<void> {
  ...
  const lines = sortedNames.map((name) => `  ${JSON.stringify(name)}: ${JSON.stringify(entries.get(name))}`);
...
// Commit: ${MODULE_REPLACEMENTS_COMMIT} (v3.0.0)
