...
export async function aggregateData(input: AggregateInput): Promise<AggregatedData> {
  ...
  const nodeWorkspaceOwners = (node: NodeInfo): Set<string> => {
    ...
    if (node.path) {
      const selfPath = path.resolve(node.path);
      // A workspace package node owns the files under its own directory.
      if (input.workspacePackagePaths?.has(selfPath)) owners.add(selfPath);
    }
  ...
  for (const node of nodes) {
    ...
    let importUsage: UsageSummary | undefined;
    let runtimeImpact: DependencyRecord['usage']['runtimeImpact'];
...
interface UsageBuildResult {
  ...
  /** Complete importing-file entries per name (topFiles caps at five). */
  entriesByName: Map<string, Array<{ file: string; count: number }>>;
}
...
function buildUsageSummary(graph: ImportGraphData, projectPath: string): UsageBuildResult {
  ...
  const entriesByName = new Map<string, Array<{ file: string; count: number }>>();
...
interface PackageStats {
  ...
  fileCount: number;
  // Measured on-disk bytes (uncompressed, excluding nested node_modules);
  // buckets sum to totalBytes.
  totalBytes: number;
  codeBytes: number;
  typesBytes: number;
  mapBytes: number;
...
interface PackageInsights {
  ...
  description?: string;
  fileCount?: number;
  installSize?: DependencyRecord['package']['installSize'];
  platform?: DependencyRecord['package']['platform'];
  hasBin: boolean;
...
async function gatherPackageInsights(
  ...
  const normalizePlatformList = (value: any): string[] | undefined => {
    if (!Array.isArray(value)) return undefined;
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          if (entry.name.endsWith('.map')) mapBytes += size;
          else if (DECLARATION_FILE_RE.test(entry.name)) typesBytes += size;
