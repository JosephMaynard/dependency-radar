...
- **Filters** — toolbar chips toggle runtime dependencies, dev dependencies, and sub-dependencies on and off, and a depth selector caps how deep the tree is expanded (Depth ≤ 1 shows just your direct dependencies and their children). Filters apply to all five layouts, but only to what renders: the dossier's impact numbers (packages in subtree, what removal frees) always come from the full workspace graph.
