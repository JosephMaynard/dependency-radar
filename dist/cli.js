...
const slideModel_1 = require("./slide/slideModel");
const slideSvg_1 = require("./slide/slideSvg");
...
function parseArgs(argv) {
    ...
    if (opts.command !== "slide" && (opts.lightMode || opts.slidePng)) {
        console.error(`Unknown option for ${opts.command}: "${opts.lightMode ? "--light-mode" : "--png"}" is only valid with the slide command.`);
        process.exit(EXIT_USAGE_OR_INCOMPLETE);
    }
    return opts;
}
 ...
 */
async function runSlideCommand(opts) {
    const result = await executeAnalysis(opts, {
        shouldWriteArtifacts: false,
        emitArtifactSummary: false,
        emitWorkspacePackageSummary: false,
    });
    const theme = opts.lightMode ? "light" : "dark";
    const svg = (0, slideSvg_1.buildSlideSvg)((0, slideModel_1.buildSlideModel)(result.aggregated), theme);
    ...
    let requested = opts.outProvided
        ? path_1.default.resolve(opts.out)
        : path_1.default.resolve(opts.project, "dependency-radar-slide.svg");
    if (opts.outProvided) {
        ...
        if ((stat && stat.isDirectory()) ||
            endsWithSeparator ||
            (!stat && !path_1.default.extname(requested))) {
            requested = path_1.default.join(requested, "dependency-radar-slide.svg");
        }
    }
    const parsed = path_1.default.parse(requested);
    const base = path_1.default.join(parsed.dir, parsed.name);
    const svgPath = base + ".svg";
    await promises_1.default.mkdir(parsed.dir, { recursive: true });
    await promises_1.default.writeFile(svgPath, svg, "utf8");
    if (!opts.quiet) {
    ...
    if (opts.slidePng) {
        ...
        if (rendered) {
            ...
            console.log(statusLine("✖", "PNG skipped: no Chrome, Edge, Chromium, or rsvg-convert found. " +
...
function slideRasterisers() {
    const candidates = [];
    if (process.env.CHROME_PATH) {
        candidates.push({ file: process.env.CHROME_PATH, kind: "chromium" });
    }
    if ((0, os_1.platform)() === "darwin") {
        candidates.push({ file: "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", kind: "chromium" }, { file: "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge", kind: "chromium" }, { file: "/Applications/Chromium.app/Contents/MacOS/Chromium", kind: "chromium" });
    }
        ...
        for (const dir of programFiles) {
            candidates.push({ file: path_1.default.join(dir, "Google", "Chrome", "Application", "chrome.exe"), kind: "chromium" }, { file: path_1.default.join(dir, "Microsoft", "Edge", "Application", "msedge.exe"), kind: "chromium" });
        }
    }
    else {
        for (const name of [
 ...
 */
async function renderSlidePng(svgPath, pngPath, svg) {
    const runQuiet = (file, args) => new Promise((resolvePromise) => {
        let child;
        try {
            child = (0, child_process_1.spawn)(file, args, { stdio: "ignore" });
        }
        catch {
    ...
    for (const candidate of slideRasterisers()) {
        if (candidate.kind === "rsvg") {
            ...
            if (ok && (await promises_1.default.stat(pngPath).catch(() => null)))
                return true;
            continue;
        }
        // Absolute paths must exist; bare names are resolved by the OS.
        if (path_1.default.isAbsolute(candidate.file)) {
            const exists = await promises_1.default.stat(candidate.file).catch(() => null);
            if (!exists)
                continue;
        }
        await promises_1.default.writeFile(wrapperPath, wrapper, "utf8");
        try {
            const ok = await runQuiet(candidate.file, [
                "--headless",
                "--disable-gpu",
                ...
                `--screenshot=${pngPath}`,
                (0, url_1.pathToFileURL)(wrapperPath).href,
            ]);
            if (ok && (await promises_1.default.stat(pngPath).catch(() => null)))
                return true;
        }
        finally {
            await promises_1.default.unlink(wrapperPath).catch(() => { });
        }
    }
    return false;
}
